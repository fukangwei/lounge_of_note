...
&emsp;&emsp;`mysyntaxhighlighter.h`如下：
...
&emsp;&emsp;`mysyntaxhighlighter.cpp`如下：
...
MainWindow::MainWindow ( QWidget *parent ) : QMainWindow ( parent ), ui ( new Ui::MainWindow ) {
    ...
    cursor.insertFrame ( frameFormat ); /* 获取光标 */
...
void MainWindow::textFind() { /* 查找文本 */
    ...
    connect ( btn, SIGNAL ( clicked() ), this, SLOT ( findNext() ) ); /* 关联信号与槽 */
...
<img src="./Qt之富文本格式/1.png" height="259" width="598">

&emsp;&emsp;富文本框中的部件其位置是跟随光标的位置。一旦回车换行，就表示创建一个新的文本块。在`Qt`的富文本处理中，提供了`QSyntaxHighlighter`类来实现语法高亮。为了实现这个功能，需要创建`QSyntaxHighlighter`的子类，然后重新实现`highlightBlock`函数，使用时直接将`QTextDocument`类对象指针作为其父部件指针，这样就可以自动调用`highlightBlock`函数。